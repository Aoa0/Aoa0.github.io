...
function create_cfp_links(){
    $('.cfp-link').click(function(){
        ...
        // Add highlight to current clicked link
...
var cfpData = {
    "2026": {
        "security": [
            ...
            ['https://sp2027.ieee-security.org/cfpapers.html', 'Oakland 2027', '2026-06-04', '2026-07-27', '13 + 5', 'Montreal', 'IEEE'],
            ['https://sp2027.ieee-security.org/cfpapers.html', 'Oakland 2027', '2026-11-10', '2027-01-18', '13 + 5', 'Montreal', 'IEEE'],
    ...
    "2019": {
        "security": [
            ['https://raid2019.cs.ucy.ac.cy/', 'RAID 2019', '2019-03-26', '2019-05-29', '', '', ''],
            ['https://www.acsac.org/2019/submissions/papers/', 'ACSAC 2019', '2019-06-08', '2019-08-23', '', '', ''],
            ['https://www.ndss-symposium.org/ndss2020/call-for-papers/', 'NDSS 2020', '2019-06-14', '2019-08-25', '', '', ''],
            ['https://www.ndss-symposium.org/ndss2020/call-for-papers/', 'NDSS 2020', '2019-09-13', '2019-12-08', '', '', ''],
            ['https://dsn2020.ntu.edu.tw/', 'DSN 2020', '2019-12-13', '2019-03-04', '', '', ''],
        ],
        "others": [
            ['https://infocom2020.ieee-infocom.org/', 'INFOCOM 2020', '2019-07-31', '2019-12-14', '10', 'Beijing', 'IEEE'],
            ['https://www.sigmetrics.org/sigmetrics2020/', 'SIGMETRICS 2020', '2019-08-08', '2019-09-30', '12 +', 'Boston', 'ACM'],
            ['https://asiaccs2020.cs.nthu.edu.tw/', 'AsiaCCS 2020', '2020-08-20', '2020-10-25', '12 +', 'London', 'ACM'],
            ['https://www2020.thewebconf.org/', 'WWW 2020', '2019-10-14', '2020-01-10', '10', 'Taibei', 'ACM'],
            ['https://saner2020.csd.uwo.ca/', 'SANER 2020', '2019-10-25', '2019-11-29', '12', 'London', 'IEEE'],
            ['https://asiaccs2020.cs.nthu.edu.tw/', 'AsiaCCS 2020', '2019-12-10', '2020-02-15', '12 +', 'London', 'ACM'],
            ['https://www.sigmobile.org/mobisys/2020/', 'MobiSys 2020', '2019-12-12', '2020-03-09', '12 +', 'Toronto', 'ACM'],
            ['https://www.sigmobile.org/mobihoc/2020/', 'MobiHoc 2020', '2019-12-13', '2020-03-29', '10', 'London', 'ACM'],
        ]
    }
};
...
function sortAllData() {
    for (var year in cfpData) {
        for (var category in cfpData[year]) {
            cfpData[year][category].sort(ascend);
        }
    }
}
...
function create_table(){
    ...
    generateTableContainers();
    
    // Create tables for each year and category
    for (var year in cfpData) {
        for (var category in cfpData[year]) {
            var tableId = category + '-' + year;
            var table = document.getElementById(tableId);
            
            if (table) {
                create_thead(table);
                create_tbody(table, cfpData[year][category]);
            }
        }
    }
}
...
// Generate table containers
function generateTableContainers() {
    var securityContainer = document.getElementById('security-tables');
    var othersContainer = document.getElementById('others-tables');
    
    // Clear containers
    securityContainer.innerHTML = '';
    othersContainer.innerHTML = '';
    
    // Generate table containers for each year
    for (var year in cfpData) {
        // Security table
        var securityTableContainer = document.createElement('div');
        securityTableContainer.className = 'cfp-table-container';
        
        var securityTable = document.createElement('table');
        securityTable.className = 'cfp cfp-' + year + ' cfp-table';
        securityTable.id = 'security-' + year;
        securityTable.style.display = 'none';
        securityTable.align = 'center';
        
        securityTableContainer.appendChild(securityTable);
        securityContainer.appendChild(securityTableContainer);
        
        // Others table
        var othersTableContainer = document.createElement('div');
        othersTableContainer.className = 'cfp-table-container';
        
        var othersTable = document.createElement('table');
        othersTable.className = 'cfp cfp-' + year + ' cfp-table';
        othersTable.id = 'others-' + year;
        othersTable.style.display = 'none';
        othersTable.align = 'center';
        
        othersTableContainer.appendChild(othersTable);
        othersContainer.appendChild(othersTableContainer);
    }
}

// Helper function: add new year data
function addYearData(year, securityData, othersData) {
    cfpData[year] = {
...
function updateYearLinks() {
    var cfpContainer = document.getElementById('cfp');
    var years = Object.keys(cfpData).sort().reverse();
    
    var linksHtml = '';
    for (var i = 0; i < years.length; i++) {
        var year = years[i];
        var yearShort = year.substring(2); // Extract "25" from "2025"
        linksHtml += '<a class="cfp-link" id="cfp-' + yearShort + '">' + year + '</a>';
        if (i < years.length - 1) {
            linksHtml += ' | ';
        }
    }
    
    cfpContainer.innerHTML = linksHtml;
